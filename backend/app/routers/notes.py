...
def get_notes_for_file_by_name(filename: str, db: Session = Depends(get_db)):
    ...
    paragraphs = db.query(Paragraph).filter(Paragraph.file_id == file.id).order_by(Paragraph.order).all()
...
def create_note(filename: str, paragraph_id: int, note: NoteCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        paragraph = db.query(Paragraph).filter(Paragraph.id == paragraph_id, Paragraph.file_id == file.id).first()
        if not paragraph:
            logger.warning(f"Paragraph {paragraph_id} not found in file {filename}.")
            raise HTTPException(status_code=404, detail="Paragraph not found in the specified file.")
        
        # Check if a note already exists for this paragraph
        existing_note = db.query(Note).filter(Note.paragraph_id == paragraph_id).first()
        if existing_note:
            logger.warning(f"Note for paragraph {paragraph_id} in file {filename} already exists.")
            raise HTTPException(status_code=400, detail="Note already exists for this paragraph.")
        ...
        db.commit()
        db.refresh(db_note)
        logger.info(f"Created note for paragraph {paragraph_id} in file {filename}.")
        return db_note
